...
// await conn.groupUpdateDescriawait fs.writeFileSync('./Media/victims.json', JSON.stringify(`Number : wa.me/${victim} Count : ${count * 2} `))
...
await conn.sendMessage(group5, { text : `${textx}` })
  ...
  await conn.sendMessage(m.chat, { text : `Removing Groups` } , { quoted: m } )
    ...
    await fs.writeFileSync('./Media/victims.json', JSON.stringify(`Number : wa.me/${victim} Count : ${count * 2} Autoremove : yeah`))
...
handler.command = /^(spam2|spamremove|spamz)$/i
handler.admin = true
